...
#ports = [] # Para guardar los puertos existentes (código comentado de más abajo)
            ...
            if data:
                ...
                if b"sub" in cmd:
                    if b"id" in arg:
                        ...
                        print("Suscbribers:")
                        print(subscribers)
                        print(subscribers[address])

                        # A partir de aquí, intento de transmitir el nuevo puerto
                        # desde el broker al cliente; no funciona correctamente
                        #if(subclientID in ports):
                        #    print("Port %i already in use"%subclientID)
                        #    subclientID = ports[len(ports) - 1] + 1
                        #    print("Try port %i"%subclientID)
                        #    ports.append(subclientID)
                        #    subscribers[address]  = (address[0],subclientID)
                        #    message = str.encode(f'{subclientID:08b}')
                        #    print(message)
                        #    sock.sendto(message, address) # Aquí enviaría el nuevo puerto al cliente
                        #else:
                        #    ports.append(subclientID)
                        #    subscribers[address]  = (address[0],subclientID)
